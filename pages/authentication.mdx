...
4. **Include headers**: Add `X-API-Key`, `X-Timestamp`, `X-Signature`, `X-Environment` required headers to your request
...
const data = await makeApiRequest("/quote", quoteRequest)
...
function makeApiRequest($endpoint, $requestBody = '') {
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => 'https://platform.arpdigital.io/api/v1' . $endpoint,
...
```bash
curl -X POST "https://platform.arpdigital.io/api/v1/quote" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <your_api_key_here>" \
  -H "X-Timestamp: <current_timestamp_in_millis>" \
  -H "X-Signature: <your_signature_key_here>" \
  -H "X-Environment: sandbox or production" \
  -d "$REQUEST_BODY: "{fromCurrency":"USD","toCurrency":"PHP","fromAmount":100}"
```
...
- For GET requests don't include a request body
